...
impl Transaction {
    pub fn coinbase_genesis(pub_key_hash: &[u8], value: u64) -> Self {
        ...
        Transaction {
            id,
            vin: vec![TXInput {
                previous_output: OutPoint { tx_id: String::new(), out_index: u32::MAX },
                pub_key:         Vec::new(),
                signature:       Vec::new(),
            }],
            vout: vec![TXOutput { value, pub_key_hash: pub_key_hash.to_vec() }],
        }
    }
    ...
    pub fn new_unsigned(inputs: Vec<(OutPoint, Vec<u8>)>, outputs: Vec<TXOutput>) -> Self {
        let vin = inputs
            ...
            .collect();
        let mut tx = Transaction { id: String::new(), vin, vout: outputs };
        tx.id = tx.signing_hash();
        tx
    }
    ...
    pub fn verify_signatures(&self) -> bool {
        ...
        let signing_id_hex = self.signing_hash();
        let msg_bytes      = hex::decode(&signing_id_hex).unwrap();

        let msg = match Message::from_digest_slice(&msg_bytes) {
            Ok(m)  => m,
            Err(_) => return false,
        };
